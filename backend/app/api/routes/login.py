...
def recover_password(email: str, session: SessionDep, background_tasks: BackgroundTasks) -> Message:
    ...
    background_tasks.add_task(
        service.send_password_reset_email, email_to=user.email, email=email, token=password_reset_token
    )
    return Message(message="Password recovery email queued")
...
def recover_password_html_content(email: str, session: SessionDep) -> Any:
    ...
    password_reset_token = generate_password_reset_token(email=email)
    service = NotificationService()
    # Recreate HTML for preview using the same service logic
    ...
    template_str = (Path(__file__).parents[2] / "email-templates" / "build" / "reset_password.html").read_text()
