...
import { randomEmail, randomPassword, randomString } from "./utils/random"
...
test("Sign up with existing email shows inline error", async ({ page }) => {
  ...
  await page.goto("/signup")
  await fillForm(page, fullName, username1, email, password, password)
  await page.getByRole("button", { name: "Sign Up" }).click()

  // Sign up again with the same email but different username
  const username2 = randomString()
  await page.goto("/signup")
  await fillForm(page, fullName, username2, email, password, password)
  await page.getByRole("button", { name: "Sign Up" }).click()
